...
export const deleteFaculty = (id) => async (dispatch) => {
    ...
    try {
        await axios.delete(`http://localhost:5000/deleteFac`);
        console.log('2222222222222222222',id);
        dispatch({ type: 'DELETE_FACULTY_SUCCESS', payload: id });
    } catch (error) {
        dispatch({ type: 'DELETE_FACULTY_FAIL', payload: error.response && error.response.data.message ? error.response.data.message : error.message });
    }
};